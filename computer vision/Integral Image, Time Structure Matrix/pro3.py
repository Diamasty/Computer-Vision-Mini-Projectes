...
class Image:
    ...
    def make_integral_image(self):
        integral = Image(self.w, self.h, self.c)

        for ch in range(self.c):
            for y in range(self.h):
                row_sum = 0.0
                for x in range(self.w):
                    val = self.get_pixel(x, y, ch)
                    row_sum += val
                    above = integral.get_pixel(x, y - 1, ch) if y > 0 else 0.0
                    integral.set_pixel(x, y, ch, row_sum + above)

        return integral
    def box_filter(self, s):
        radius = s // 2
        integral = self.make_integral_image()
        out = Image(self.w, self.h, self.c)

        for ch in range(self.c):
            for y in range(self.h):
                for x in range(self.w):
                    x0 = max(x - radius, 0)
                    x1 = min(x + radius, self.w - 1)
                    y0 = max(y - radius, 0)
                    y1 = min(y + radius, self.h - 1)

                    A = integral.get_pixel(x1, y1, ch)
                    B = integral.get_pixel(x0 - 1, y1, ch) if x0 > 0 else 0
                    C = integral.get_pixel(x1, y0 - 1, ch) if y0 > 0 else 0
                    D = integral.get_pixel(x0 - 1, y0 - 1, ch) if x0 > 0 and y0 > 0 else 0

                    region_sum = A - B - C + D
                    area = (x1 - x0 + 1) * (y1 - y0 + 1)
                    avg = region_sum / area

                    out.set_pixel(x, y, ch, avg)

        return out
    def time_structure_matrix(self, prev):
        assert self.w == prev.w and self.h == prev.h, "Image dimensions must match."

        w, h = self.w, self.h
        S = Image(w, h, 6)  # 6 channels for structure components

        # Compute spatial gradients
        gx_filter = make_gx_filter()
        gy_filter = make_gy_filter()
        Ix = convolve_image(self, gx_filter)
        Iy = convolve_image(self, gy_filter)

        for y in range(h):
            for x in range(w):
                ix = Ix.get_pixel(x, y, 0)
                iy = Iy.get_pixel(x, y, 0)
                it = self.get_pixel(x, y, 0) - prev.get_pixel(x, y, 0)

                S.set_pixel(x, y, 0, ix * ix)   # Ix^2
                S.set_pixel(x, y, 1, ix * iy)   # Ix*Iy
                S.set_pixel(x, y, 2, iy * iy)   # Iy^2
                S.set_pixel(x, y, 3, ix * it)   # Ix*It
                S.set_pixel(x, y, 4, iy * it)   # Iy*It
                S.set_pixel(x, y, 5, it * it)   # It^2 (optional)
